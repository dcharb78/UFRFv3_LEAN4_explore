...
/-- The full cycle, using the derived length from Foundation. -/
abbrev FullCycle := Fin UFRF.Foundation.derived_cycle_length

/-- The Observer occupies position 0 (the entry/mediator). -/
def observer : FullCycle := ⟨0, by decide⟩
...
theorem measured_count : measuredPositions.card = 12 := by decide
...
✅ PROVEN
-/
theorem base10_from_rest :
    (Finset.univ.filter (fun p : FullCycle => p.val < 10)).card = 10 := by decide

def perceivedPositions : Finset FullCycle :=
  Finset.univ.filter (fun p => p.val < 10)

theorem perceived_count : perceivedPositions.card = 10 := base10_from_rest
...
-/
theorem positions_per_circle : Fintype.card FullCycle = 13 := by decide
